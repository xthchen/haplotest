...
adapted.cmh.test.est = function(freq, Ne, gen, cov,repli){
  k = length(gen)
  #no. timepoints
  ...
  for (i in 1:repli){
    ...
    if (!is.matrix(Ne)){
      ...
      tchi_num = cbind(tchi_num, n_0^2*n_T^2*((f_0*(1-f_T)-(1-f_0)*f_T)^2))
      tchi_den = cbind(tchi_den, ((n_0^2*n_T*(f_T*(1-f_T)+(n_T-1)*drift))+n_T^2*n_0*f_0*(1-f_0)))
    }else{
